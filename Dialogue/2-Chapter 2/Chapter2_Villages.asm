;	Village events
;	Numbered top to bottom, 1 = top

dialogueCh2Village1

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldMan
	.byte StartText
.text	"Beware of Bishop Clement,"
	.byte NewLine
.text	"the lord of Mackily Castle."
	.byte WaitForA
	.byte ScrollText

.text	"That dastard wields a magical staff"
	.byte NewLine
.text	"that they call a Sleep staff."
	.byte WaitForA
	.byte ScrollText

.text	"If you're not careful, you'll find yourself"
	.byte NewLine
.text	"snoozin' and wide open to bein' attacked."
	.byte WaitForA

	.byte EndText


dialogueCh2Village2

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_YoungWoman
	.byte StartText
.text	"Hey, thanks for saving us!"
	.byte NewLine
.text	"I want you to have this,"
	.byte NewLine
.text	"one of my favorite things."
	.byte WaitForA
	.byte ScrollText

.text	"It's a pretty little bracelet, huh?"
	.byte NewLine
.text	"Wear it at the shops and the nice man"
	.byte NewLine
.text	"there'll be happy to give ya a discount!"
	.byte WaitForA

	.byte EndText


dialogueCh2Village3

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldMan
	.byte StartText
.text	"The late King Imca was truly a great man."
	.byte NewLine
.text	"His sudden passing has been"
	.byte NewLine
.text	"nothin' but devastating for us all."
	.byte WaitForA
	.byte ScrollText

.text	"He's barely been buried for long"
	.byte NewLine
.text	"and his son Chagall's already"
	.byte NewLine
.text	"runnin' the country into the ground!"
	.byte WaitForA
	.byte ScrollText

.text	"If Lord Eldigan had a claim to the throne,"
	.byte NewLine
.text	"somehow, then we wouldn't be in this mess..."
	.byte WaitForA

	.byte EndText


dialogueCh2Village4

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_Woman
	.byte StartText
.text	"Lord Eldigan is a sincere and righteous man,"
	.byte NewLine
.text	"but alas, his sincerity's never been welcome"
	.byte NewLine
.text	"to the ears of King Chagall."
	.byte WaitForA
	.byte ScrollText

.text	"That's gotta be why he was locked up."
	.byte NewLine
.text	"Please, for the good of all Agustria..."
	.byte NewLine
.text	"Help Lord Eldigan!"
	.byte WaitForA

	.byte EndText


dialogueCh2Village5

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_YoungMan
	.byte StartText
.text	"There's still gotta be descendants"
	.byte NewLine
.text	"of the dark lord Loptyr livin' somewhere, right?"
	.byte WaitForA
	.byte ScrollText

.text	"Folk in the big cities like Agusty an' Mackily"
	.byte NewLine
.text	"certainly seem to think so. Every year they do"
	.byte NewLine
.text	"these huge hunts for Loptyr's spawn."
	.byte WaitForA
	.byte ScrollText

.text	"They arrest a ton of people, then burn 'em all"
	.byte NewLine
.text	"at the stake, even if they're innocent..."
	.byte NewLine
.text	"Madness, I tell ya!"
	.byte WaitForA

	.byte EndText


dialogueCh2Village6

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_BigNose2
	.byte StartText
.text	"Anphony's Lord Macbeth is the foulest,"
	.byte NewLine
.text	"most gold-obsessed man there is."
	.byte NewLine
.text	"He does nothin' but torment us for his own profit."
	.byte WaitForA
	.byte ScrollText

.text	"I wish I could kill 'em with my bare hands!"
	.byte WaitForA

	.byte EndText


dialogueCh2Village7

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_BigNose2
	.byte StartText
.text	"The great Lord Eldigan of Nordion"
	.byte NewLine
.text	"is a descendant of Hezul the Brave,"
	.byte NewLine
.text	"one of the legendary Twelve Crusaders."
	.byte WaitForA
	.byte ScrollText

.text	"He wields Hezul's old weapon,"
	.byte NewLine
.text	"Mystletainn, the demon blade."
	.byte WaitForA
	.byte ScrollText

.text	"They say it's as strong as Isaach's Balmung"
	.byte NewLine
.text	"and Chalphy's Tyrfing..."
	.byte NewLine
.text	"The three mightiest blades in all of creation."
	.byte WaitForA

	.byte EndText


dialogueCh2Village8

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_Woman
	.byte StartText
.text	"Psst!"
	.byte NewLine
.text	"Wanna hear somethin' saucy?"
	.byte WaitForA
	.byte ScrollText

.text	"Y'know how Lord Eldigan has a li'l sister, right?"
	.byte NewLine
.text	"Well, lookin' at 'em, wouldn't ya say"
	.byte NewLine
.text	"there's more to it than that?"
	.byte WaitForA
	.byte ScrollText

.text	"If ya know what I mean... Ohoho!"
	.byte NewLine
.text	"I love me a li'l bit of gossip!"
	.byte WaitForA
	.byte ScrollText

.text	"Hmm?"
	.byte NewLine
.text	"I've better things to do than run me mouth, y'say?"
	.byte NewLine
.text	"Fine, then."
	.byte WaitForA

	.byte EndText


dialogueCh2Village9

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_Woman
	.byte StartText
.text	"War's the most dreadful thing, and 'tis always"
	.byte NewLine
.text	"the innocent and helpless who suffer most for it."
	.byte NewLine
.text	"Please... go home to your families."
	.byte WaitForA

	.byte EndText


dialogueCh2Village10

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldMan
	.byte StartText
.text	"We can't thank you enough for helpin' us."
	.byte NewLine
.text	"This here sword's the best we can do."
	.byte WaitForA
	.byte ScrollText

.text	"It's an unusual sort, innit?"
	.byte NewLine
.text	"They call it an armorslayer. In the right hands"
	.byte NewLine
.text	"it'll cleave right through the thickest armor."
	.byte WaitForA
	.byte ScrollText

.text	"If your hands aren't the right hands, though?"
	.byte NewLine
.text	"Maybe it can make ya a quick buck"
	.byte NewLine
.text	"at a pawn shop in town."
	.byte WaitForA

	.byte EndText