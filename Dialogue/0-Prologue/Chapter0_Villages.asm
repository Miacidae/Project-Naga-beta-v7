	;villages

dialogueVillageNearestChalphy

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_BigNose2

	.byte StartText 
.text	"Oh, thank the gods!"
	.byte NewLine
.text	"We're saved!"
	.byte NewLine
.text	"We can't thank you enough!"
	.byte WaitForA
	.byte ScrollText

.text	"Here - the whole village did their bit"
	.byte NewLine
.text	"to raise some money for you."
	.byte NewLine
.text	"It's not much, but we hope it helps out there."
	.byte WaitForA
	.byte ScrollText

.text	"Be sure to lend the other villages a hand, too."
	.byte NewLine
.text	"I bet they'll be happy to help you out right back."
	.byte WaitForA
	.byte EndText


dialogueVillageNorthOfJungby

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldMan

	.byte StartText 
.text	"Pay attention, kid! Weapons aren't created"
	.byte NewLine
.text	"equal. See, swords best axes, axes best"
	.byte NewLine
.text	"lances, and lances best swords."
	.byte WaitForA
	.byte ScrollText

.text	"This right here's why you've gotta use your"
	.byte NewLine
.text	"head before you blunder into battle."
	.byte WaitForA
	.byte ScrollText

.text	"Speaking of battle, if you ever find yourself"
	.byte NewLine
.text	"hurting out there, a chapel is the best place"
	.byte NewLine
.text	"to go."
	.byte WaitForA
	.byte ScrollText

.text	"It ain't free, mind. Every bit of health you regain"
	.byte NewLine
.text	"there'll cost five gold pieces."
	.byte NewLine
.text	"But you can't put a price on your health."
	.byte WaitForA
	.byte EndText


dialogueVillageNearestJungby

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldWoman

	.byte StartText 
.text	"Oh? You came to save us?"
	.byte NewLine
.text	"I was so sure this was the end!"
	.byte NewLine
.text	"Take this little trinket. It's the least I can do."
	.byte WaitForA
	.byte ScrollText

.text	"They call it a speed ring."
	.byte NewLine
.text	"Slip this magic band on and"
	.byte NewLine
.text	"you'll be as nimble as a feather!"
	.byte WaitForA
	.byte ScrollText

.text	"Go on, try her out!"
	.byte NewLine
.text	"Your enemies'll be too slow to hit you!"
	.byte WaitForA
	.byte EndText


dialogueVillageNorthwestJungby

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldMan

	.byte StartText 
.text	"The master archers of House Jungby"
	.byte NewLine
.text	"all bear descent from the archery god, Ullur."
	.byte WaitForA
	.byte ScrollText

.text	"Our noble duke, Lord Ring, has twin daughters"
	.byte NewLine
.text	"as well as a younger son."
	.byte WaitForA
	.byte ScrollText

.text	"Alas, the duke lost one of the girls to pirates,"
	.byte NewLine
.text	"years and years ago..."
	.byte NewLine
.text	"She was just a little lass..."
	.byte WaitForA
	.byte ScrollText

.text	"Lady Aideen's still here, but the son"
	.byte NewLine
.text	"Lord André is away with his lord father"
	.byte NewLine
.text	"on the Isaach campaign."
	.byte WaitForA
	.byte ScrollText

.text	"Rumor has it, though, that the two aren't"
	.byte NewLine
.text	"getting along well now that the boy's grown..."
	.byte WaitForA
	.byte EndText


dialogueVillageNorthernmost

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_Woman

	.byte StartText 
.text	"The lords of this land, House Chalphy,"
	.byte NewLine
.text	"have been revered as most magnificent"
	.byte NewLine
.text	"swordsmen for generations."
	.byte WaitForA
	.byte ScrollText

.text	"It's no wonder, being that they bear the bloodline"
	.byte NewLine
.text	"of Baldur, the god of light of old."
	.byte WaitForA
	.byte ScrollText

.text	"Those Verdane thugs are lucky Lord Byron"
	.byte NewLine
.text	"isn't around with his holy sword, Tyrfing."
	.byte NewLine
.text	"They wouldn't stand a chance!"
	.byte WaitForA
	.byte EndText
