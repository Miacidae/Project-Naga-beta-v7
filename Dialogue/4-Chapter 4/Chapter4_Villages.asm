;	Village events

dialogueCh4VillageSouthernmost

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldMan
	.byte StartText
.text	"Magic is governed by a hierarchy of power,"
	.byte NewLine
.text	"just like weapons."
	.byte WaitForA
	.byte ScrollText
	
.text	"Fire magic bests wind, wind bests thunder,"
	.byte NewLine
.text	"and thunder bests fire."
	.byte WaitForA
	.byte ScrollText
	
.text	"However, dark and light magic have"
	.byte NewLine
.text	"an advantage over all of the others."
	.byte NewLine
.text	"Think your strategies through with care."
	.byte WaitForA

	.byte EndText


dialogueCh4VillageWesternmost

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldMan
	.byte StartText
.text	"Only a direct holy descendant can inherit"
	.byte NewLine
.text	"the full breadth of the power of the gods."
	.byte WaitForA
	.byte ScrollText
	
.text	"And only they'll be able to wield"
	.byte NewLine
.text	"their family's divine heirlooms."
	.byte WaitForA
	.byte ScrollText
	
.text	"However, if kin intersect, their offspring'll"
	.byte NewLine
.text	"have their exalted blood enriched"
	.byte NewLine
.text	"and'll reap almighty powers."
	.byte WaitForA
	.byte ScrollText
	
.text	"Needless to say, mind, doin' that"
	.byte NewLine
.text	"is condemned and forbidden..."
	.byte WaitForA

	.byte EndText


dialogueCh4VillageLowerMiddle

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldMan
	.byte StartText
.text	"Bein' of holy ancestry can bestow"
	.byte NewLine
.text	"a number of powers onto ya."
	.byte WaitForA
	.byte ScrollText
	
.text	"Divine heirs can wield even the"
	.byte NewLine
.text	"most advanced weapons with ease,"
	.byte NewLine
.text	"and'll have an easier time growin' stronger."
	.byte WaitForA
	.byte ScrollText
	
.text	"'Course, it's not every day you meet someone"
	.byte NewLine
.text	"blessed with that sorta blood."
	.byte WaitForA

	.byte EndText


dialogueCh4VillageUpperMiddleRight

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldMan
	.byte StartText
.text	"The Silesse royal family wields the power of wind,"
	.byte NewLine
.text	"as bestowed by the wind god, Forseti."
	.byte WaitForA
	.byte ScrollText
	
.text	"House Velthomer wields the power of fire,"
	.byte NewLine
.text	"as bestowed by the fire goddess, Vala."
	.byte WaitForA
	.byte ScrollText
	
.text	"And House Freege wields thunder powers,"
	.byte NewLine
.text	"as bestowed by the thunder god, Tordo."
	.byte WaitForA

	.byte EndText


dialogueCh4VillageUpperMiddleLeft

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldMan
	.byte StartText
.text	"The sinister powers of the dark god Loptyr"
	.byte NewLine
.text	"can be fought only with the glorious powers"
	.byte NewLine
.text	"of Naga, our great god."
	.byte WaitForA
	.byte ScrollText
	
.text	"Only Grannvale's House Belhalla,"
	.byte NewLine
.text	"the heirs to Saint Heim, inherit Naga's power."
	.byte WaitForA

	.byte EndText


dialogueCh4VillageWestOfThove_BridgeDown

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_BigNose2
	.byte StartText
.text	"Y'know how there's a drawbridge over the"
	.byte NewLine
.text	"Thove River? Yeah, that's under the control"
	.byte NewLine
.text	"of Duke Maios, too."
	.byte WaitForA
	.byte ScrollText
	
.text	"If it's still drawn and you don't have the key,"
	.byte NewLine
.text	"you're not getting across."
	.byte WaitForA
	.byte ScrollText
	
.text	"Mind you, if you have a thief's sticky fingers"
	.byte NewLine
.text	"at your service, that might not be a problem..."
	.byte WaitForA
	.byte ScrollText
	
.text	"What's that?"
	.byte NewLine
.text	"You got the drawbridge lowered?"
	.byte NewLine
.text	"How'd ya know to do that?"
	.byte WaitForA

	.byte EndText


dialogueCh4VillageWestOfThove_BridgeUp

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_BigNose2
	.byte StartText
.text	"Y'know how there's a drawbridge over the"
	.byte NewLine
.text	"Thove River? Yeah, that's under the control"
	.byte NewLine
.text	"of Duke Maios, too."
	.byte WaitForA
	.byte ScrollText
	
.text	"If it's still drawn and you don't have the key,"
	.byte NewLine
.text	"you're not getting across."
	.byte WaitForA
	.byte ScrollText
	
.text	"Mind you, if you have a thief's sticky fingers"
	.byte NewLine
.text	"at your service, that might not be a problem..."
	.byte WaitForA

	.byte EndText


dialogueCh4VillageSecondWestOfThove

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldMan
	.byte StartText
.text	"Legend has it that all direct heirs to"
	.byte NewLine
.text	"the Crusaders bear exalted brands on their bodies,"
	.byte NewLine
.text	"which appear on them in their infancy."
	.byte WaitForA
	.byte ScrollText
	
.text	"Prince Kurth's daughter has her Naga brand"
	.byte NewLine
.text	"on her forehead."
	.byte WaitForA
	.byte ScrollText
	
.text	"Actually, that was how they recognized her"
	.byte NewLine
.text	"as a direct heiress to Saint Heim."
	.byte WaitForA

	.byte EndText


dialogueCh4VillageSecondWestOfThove_Sylvia1

	.byte LeftSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_BigNose2
	
	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Sylvia
	
	.byte LeftSlot
	.byte StartText
.text	"Oh! Lord Sigurd's army is here to save us all!"
	.byte NewLine
.text	"Hang on..."
	.byte NewLine
.text	"Aren't you a dancer, miss?"
	.byte WaitForA
	.byte ScrollText
	
.text	"If you don't mind, perhaps you could"
	.byte NewLine
.text	"do a little dance for us?"
	.byte WaitForA
	.byte ScrollText
	
.text	"It's been so long since we've had time"
	.byte NewLine
.text	"for respite or fun..."

	.byte RightSlot		;Sylvia
	.word PauseText
	.byte $10
.text	"Yep, not a problem!"
	.byte NewLine
.text	"If you think it'll help everyone cheer up a bit,"
	.byte NewLine
.text	"then I'm gonna dance my feet off!"
	.byte WaitForA
	.word ScrollBoth
	
.text	"...Okay! Heeeere we go!"
	.byte WaitForA

	.byte EndText


dialogueCh4VillageSecondWestOfThove_Sylvia2

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Sylvia
	
	.byte RightSlot
	.byte StartText
.text	"Hah!"
	.word PauseText
	.byte $64
	
	.byte EndText


dialogueCh4VillageSecondWestOfThove_Sylvia3

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Sylvia
	.byte RightSlot
	.byte StartText
.text	"Yeah!"
	.word PauseText
	.byte $64
	
	.byte EndText
	
	
dialogueCh4VillageSecondWestOfThove_Sylvia4

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Sylvia
	.byte RightSlot
	.byte StartText
.text	"Hyaaaah!"
	.word PauseText
	.byte $64
	
	.byte EndText


dialogueCh4VillageSecondWestOfThove_Sylvia5

	.byte LeftSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_BigNose2
	
	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Sylvia
	.byte RightSlot
	.byte StartText
.text	"Whew..."
	.byte NewLine
.text	"Well, how 'bout that?"
	.byte NewLine
.text	"Everyone feelin' a little better?"
	.byte WaitForA
	.byte ScrollText
	
	.byte LeftSlot
.text	"Oh! What fantastic dancin'!"
	.byte NewLine
.text	"I dunno how, but now I feel like"
	.byte NewLine
.text	"I'm ready to give life my all!"
	.byte WaitForA
	.byte ScrollText
	
.text	"I can't thank you enough, miss."
	.byte NewLine
.text	"Here, perhaps this magic sword'll help ya out."
	.byte NewLine
.text	"It oughta do a fine job of keeping ya safe."
	.byte WaitForA

	.byte EndText


dialogueCh4VillageFarNorthWestOfZaxon

	.byte RightSlot
	.word OpenBox
	.word LoadPortrait
	.word portrait_Civilian_OldMan
	.byte StartText
.text	"Should Prince Kurth's daughter bear a son,"
	.byte NewLine
.text	"that son would be heir to Grannvale's throne."
	.byte WaitForA
	.byte ScrollText
	
.text	"I daresay this makes Lord Arvis,"
	.byte NewLine
.text	"husband to the princess,"
	.byte NewLine
.text	"quite the influential figure."
	.byte WaitForA

	.byte EndText